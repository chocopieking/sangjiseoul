...
찾을 수 없는 값은 빈 문자열 또는 0으로 두세요.`,
  },
  {
    ...
    fields:[
      ...
      {k:"contractName", label:"용역명"},
      {k:"issuer",  label:"발주처"},
      {k:"jointVenture", label:"공동이행 현황(업체명·비율·금액)"},
      {k:"subcontract", label:"분담이행 현황(업체명/공종·비율·금액)"},
      {k:"bondInfo", label:"계약보증금액/보증기간/보증처"},
      {k:"specialTerms", label:"계약 특수조건"},
      {k:"contactName",  label:"작성자"},
      {k:"contactPhone", label:"작성자 소속/연락처"},
    ],
    prompt:`이 문서는 공공 또는 민간 발주 프로젝트의 내부결재용 "계약검토/체결 보고서"입니다. 발주처·계약일·완료일 밑에 "공동이행"표(업체명/비율/금액)와 "분담이행"표(업체명 또는 업체명/공종·비율·금액)가 따로 있는 경우가 많습니다. 아래 JSON 형식으로만 응답하세요(설명 없이 JSON만):
{"docNo":"계약번호(없으면 빈문자열)","amount":문서에 "상지" 또는 "상지엔지니어링/상지서울/상지건축" 몫으로 표기된 계약금액(숫자, 원 단위 — 컨소시엄 전체 합계금액이 아니라 우리 회사 몫만),"startDate":"계약일 YYYY-MM-DD","orderDate":"착수일 YYYY-MM-DD(없으면 빈문자열)","endDate":"완료일/계약기간 종료일/총완수일 YYYY-MM-DD(없으면 빈문자열)","contractName":"용역명","issuer":"발주처명","jointVenture":"공동이행 표에 있는 업체명·비율·금액을 '업체명 비율% 금액원' 형식으로 줄바꿈 없이 쉼표로 나열(없으면 빈문자열)","subcontract":"분담이행 표에 있는 업체명(공종)·비율·금액을 같은 방식으로 나열(없으면 빈문자열)","bondInfo":"계약보증금액과 보증기간, 보증처(공제조합 등)를 한 줄로(없으면 빈문자열)","specialTerms":"용역범위·특수조건을 1~2줄로 요약(없으면 빈문자열)","contactName":"작성자 이름(없으면 빈문자열)","contactPhone":"작성자 소속(부서명, 없으면 빈문자열)"}
찾을 수 없는 값은 빈 문자열 또는 0으로 두세요. amount는 반드시 상지(우리 회사) 몫 금액이어야 하며, 컨소시엄 전체 합계금액을 넣지 마세요. 공동이행/분담이행 구분이 없이 "도급현황" 하나로만 되어 있으면 jointVenture 쪽에 전부 넣으세요.`,
  },
]
