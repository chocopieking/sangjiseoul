...
const S = {
  ...
  td:(a="right")=>({padding:"9px 12px",borderBottom:"0.5px solid var(--color-border-tertiary,#eee)",textAlign:a,fontSize:14.8,verticalAlign:"middle"}),
  ...
  inp:(w=120)=>({width:w,padding:"7px 9px",border:"1px solid var(--color-border-secondary,#ccc)",borderRadius:8,fontSize:14.3,fontFamily:"inherit",background:"#fff",color:"#222"}),
  btn:(bg=C.navyM,fg="#fff")=>({padding:"9px 16px",background:bg,color:fg,border:"none",borderRadius:10,fontSize:14.3,fontWeight:700,cursor:"pointer",display:"inline-flex",alignItems:"center",gap:6}),
  lbl:()=>({display:"block",fontSize:12,color:C.gray,fontWeight:600,marginBottom:3}),
}
const cardTitle = {fontSize:17.6,fontWeight:700,marginBottom:4,letterSpacing:-.2}
const cardNote  = {fontSize:13.2,color:C.gray,marginBottom:12}
      ...
      <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap",alignItems:"center"}}>
        ...
        <div style={{marginLeft:"auto",display:"flex",gap:6,flexWrap:"wrap"}}>
          <button onClick={downloadVendors}
            style={{...S.btn("var(--color-background-secondary,#f3f3f0)","var(--color-text-primary,#222)"),fontSize:13.8,padding:"7px 13px"}}>
            ⬇ 전체 다운로드
          </button>
          {canWrite&&<label style={{...S.btn(C.navyM),fontSize:13.8,padding:"7px 13px",cursor:"pointer"}}>
            ⬆ 협력업체 업로드
            <input type="file" accept=".xlsx,.xls" style={{display:"none"}} onChange={uploadVendors}/>
          </label>}
          {canWrite&&<label style={{...S.btn("var(--color-background-secondary,#f3f3f0)","#D97706"),fontSize:13.8,padding:"7px 13px",cursor:"pointer",border:"1.5px solid #D97706"}}>
            ⬆ 💰 외주비 업로드
      ...
      <div style={{marginBottom:10}}>
        <div style={{display:"flex",gap:8,marginBottom:8,flexWrap:"wrap",alignItems:"center"}}>
          <input value={search} onChange={e=>{setSearch(e.target.value);setF("name",e.target.value)}}
            placeholder="🔍 업체명 검색..."
            style={{...S.inp(220),textAlign:"left",padding:"9px 14px",fontSize:15.4,flex:1}}/>
          <button onClick={()=>setShowFilter(v=>!v)}
            style={{...S.btn(showFilter?C.navyM:C.navyL,showFilter?"#fff":C.navyM),padding:"8px 14px",fontSize:14.3}}>
            {showFilter?"▲ 간단검색":"▼ 상세검색"}
          </button>
          <div style={{fontSize:14.3,color:C.gray,fontWeight:600,whiteSpace:"nowrap"}}>
          ...
          <div style={{background:"#F8FAFC",borderRadius:12,border:"1px solid #E5E7EB",padding:"14px 16px",marginBottom:8}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:10}}>
              {[["업체명","name","업체명 검색"],["주요분야/업무구분","field","예: 건축, 구조, 기계"],["대표자/담당자","rep","이름 검색"]].map(([l,k,ph])=>(
                <div key={k}>
                  <label style={{fontSize:12.6,fontWeight:700,color:C.navyM,display:"block",marginBottom:3}}>{l}</label>
                  <input value={filters[k]} onChange={e=>setF(k,e.target.value)} placeholder={ph}
                    style={{...S.inp("100%"),textAlign:"left",padding:"7px 10px",fontSize:14.3,width:"100%",boxSizing:"border-box"}}/>
                </div>
              ))}
            </div>
            ...
            <button onClick={()=>{setFilters({name:"",field:"",rep:"",tel:"",addr:""});setSearch("");setPage(1)}}
              style={{marginTop:10,padding:"5px 14px",background:"#F3F4F6",color:"#6B7280",border:"none",borderRadius:7,fontSize:13.8,cursor:"pointer"}}>
              초기화
            </button>
          </div>
        )}
          ...
          {[["name","업체명순"],["projects","프로젝트 많은순"],["payment","외주비 많은순"]].map(([v,l])=>(
            <button key={v} onClick={()=>setSortBy(v)}
              style={{padding:"4px 12px",border:`1.5px solid ${sortBy===v?C.navyM:"#E5E7EB"}`,borderRadius:7,
                fontSize:13.2,cursor:"pointer",background:sortBy===v?C.navyL:"#fff",color:sortBy===v?C.navyM:"#6B7280",fontWeight:sortBy===v?700:400}}>
              {l}
            </button>
          ))}
        </div>
          ...
          <tbody>
            ...
            {pageData.map((v,i)=>(
              <tr key={v.id||v.name} onClick={()=>onSelect(v.dirEntry||v)}
                ...
                <td style={{...S.td("left"),fontSize:13.8,color:C.gray}}>
                  {v.bizType||((v.cats||[]).join(", "))||"-"}
                </td>
                <td style={{...S.td("left"),fontSize:13.8}}>
                  {v.rep||"-"}
                  {v.contact&&v.contact!==v.rep&&<span style={{fontSize:12,color:C.gray}}> / {v.contact}</span>}
                </td>
                <td style={{...S.td("left"),fontSize:13.8,color:C.gray}}>
                  {v.tel||v.contactTel||"-"}
                </td>
                ...
                <td style={{...S.td("left"),fontSize:12.6,color:C.gray,maxWidth:180,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
                  {v.addr||"-"}
                </td>
              </tr>
            ))}
      ...
      {totalPages>1&&(
        <div style={{display:"flex",justifyContent:"center",gap:6,marginTop:14,flexWrap:"wrap"}}>
          <button onClick={()=>setPage(1)} disabled={page===1}
            style={{padding:"5px 10px",border:"1px solid #E5E7EB",borderRadius:7,fontSize:13.2,cursor:page===1?"not-allowed":"pointer",color:page===1?C.gray:C.navyM}}>«</button>
          ...
          {Array.from({length:Math.min(7,totalPages)},(_, i)=>{
            ...
            return p<=totalPages?(
              <button key={p} onClick={()=>setPage(p)}
                style={{padding:"5px 12px",border:`1.5px solid ${page===p?C.navyM:"#E5E7EB"}`,borderRadius:7,fontSize:13.8,
          ...
          <button onClick={()=>setPage(p=>Math.min(totalPages,p+1))} disabled={page===totalPages}
            style={{padding:"5px 10px",border:"1px solid #E5E7EB",borderRadius:7,fontSize:13.2,cursor:page===totalPages?"not-allowed":"pointer",color:page===totalPages?C.gray:C.navyM}}>›</button>
          <button onClick={()=>setPage(totalPages)} disabled={page===totalPages}
            style={{padding:"5px 10px",border:"1px solid #E5E7EB",borderRadius:7,fontSize:13.2,cursor:page===totalPages?"not-allowed":"pointer",color:page===totalPages?C.gray:C.navyM}}>»</button>
          <span style={{fontSize:13.8,color:C.gray,padding:"5px 0",alignSelf:"center"}}>
    ...
    <div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14}}>
          {FIELDS.map(([k,l])=>(
            <div key={k}>
              ...
              {editing
                ...
                :<div style={{fontSize:15.4,fontWeight:info[k]?600:400,color:info[k]?undefined:C.gray,padding:"7px 0"}}>{info[k]||"미입력"}</div>}
            </div>
          ))}
          <div style={{gridColumn:"1/-1"}}>
            ...
            {editing
              ...
              :<div style={{fontSize:14.3,color:info.note?undefined:C.gray}}>{info.note||"-"}</div>}
          </div>
        </div>
      </SCard>
      ...
      <div style={{display:"flex",gap:4,marginBottom:14,borderBottom:"2px solid #E5E7EB"}}>
        {[["info","🏗 프로젝트·지급"],["plan","📅 지급계획 편집"],["docs","📎 문서보관"],["history","📅 사안기록"],["payments","💰 지급내역"]].map(([id,lbl])=>(
          <button key={id} onClick={()=>setSubTab(id)} style={{padding:"9px 18px",border:"none",background:"none",fontSize:14.8,fontWeight:700,cursor:"pointer",
      ...
      {subTab==="history"&&(
        <SCard title="📅 사안별 날짜 기록" note="계약·변경·대금지급 등 주요 사안을 날짜별로 기록합니다">
          ...
          {/* 타임라인 */}
          {history.length===0
            ?<div style={{padding:"24px",textAlign:"center",color:"#9CA3AF",fontSize:14.3}}>등록된 기록이 없습니다.</div>
            :<div style={{position:"relative",paddingLeft:8}}>
              ...
              {[...history].reverse().map(h=>(
                <div key={h.id} style={{display:"flex",gap:14,marginBottom:10,alignItems:"flex-start"}}>
                  <div style={{width:112,flexShrink:0,textAlign:"right",paddingTop:3,fontSize:13.8,fontWeight:700,color:"#374151"}}>{h.date}</div>
                    ...
                    {editHId===h.id
                      ?<div style={{display:"flex",flexDirection:"column",gap:7}}>
                        ...
                        <div style={{display:"flex",gap:6}}>
                          <button onClick={saveHEdit} style={{...S.btn(C.green),padding:"5px 12px",fontSize:13.2}}>저장</button>
                          <button onClick={()=>setEditHId(null)} style={{...S.btn(C.grayL,C.gray),padding:"5px 12px",fontSize:13.2}}>취소</button>
                        </div>
                      </div>
                      :<>
                        <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                          <span style={{fontSize:12.6,padding:"2px 8px",borderRadius:10,background:(catColor[h.category]||"#9CA3AF")+"22",color:catColor[h.category]||"#9CA3AF",fontWeight:700}}>{h.category}</span>
                          <span style={{fontSize:15.4,fontWeight:700,color:"#111827",flex:1}}>{h.content}</span>
                          {canWrite&&<div style={{display:"flex",gap:4,marginLeft:"auto"}}>
                            <button onClick={()=>{setEditHId(h.id);setEHD({date:h.date,category:h.category,content:h.content,memo:h.memo||""})}} style={{...S.btn(C.navyL,C.navyM),padding:"3px 9px",fontSize:12}}>수정</button>
                            <button onClick={()=>removeHistory(h.id)} style={{...S.btn(C.redL,C.red),padding:"3px 9px",fontSize:12}}>삭제</button>
                          </div>}
                        </div>
                        {h.memo&&<div style={{fontSize:13.8,color:"#6B7280",background:"#F8FAFC",borderRadius:7,padding:"5px 9px",marginTop:4}}>📝 {h.memo}</div>}
                        <div style={{fontSize:11.6,color:"#9CA3AF",marginTop:4}}>
      ...
      {subTab==="payments"&&(
        <div>
                  ...
                  <details key={pi} style={{marginBottom:10,border:"1px solid #E5E7EB",borderRadius:10,overflow:"hidden"}}>
                    <summary style={{padding:"12px 16px",cursor:"pointer",background:"#F8FAFC",display:"flex",gap:10,alignItems:"center",justifyContent:"space-between",listStyle:"none"}}>
                      <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
                        <span style={{fontSize:12.6,padding:"2px 8px",borderRadius:7,background:"#E3F6F3",color:"#0E9C8C",fontWeight:700}}>{ph.type}</span>
                        <span style={{fontSize:15.4,fontWeight:700,color:"#111827"}}>{ph.project?.slice(0,35)}</span>
                      </div>
                      <div style={{display:"flex",gap:14,alignItems:"center",flexShrink:0}}>
                        <div style={{textAlign:"right"}}>
                          <div style={{fontSize:12,color:"#9CA3AF"}}>총 용역금액</div>
                          <div style={{fontSize:14.8,fontWeight:800,color:"#0B6E63"}}>{(ph.totalAmt/1e6).toFixed(1)}백만원</div>
                        </div>
                        <div style={{textAlign:"right"}}>
                          <div style={{fontSize:12,color:"#9CA3AF"}}>지급 합계</div>
                          <div style={{fontSize:14.8,fontWeight:800,color:"#059669"}}>{ph.paidSum>0?(ph.paidSum/1e6).toFixed(1)+"백만원":"-"}</div>
                        </div>
                        <div style={{textAlign:"right"}}>
                          <div style={{fontSize:12,color:"#9CA3AF"}}>잔액</div>
                          <div style={{fontSize:14.8,fontWeight:800,color:ph.remain>0?"#DC2626":"#9CA3AF"}}>{ph.remain>0?(ph.remain/1e6).toFixed(1)+"백만원":"-"}</div>
                        </div>
                        {paidRate>0&&(
                          <div style={{width:44,height:44,position:"relative",flexShrink:0}}>
                            ...
                            <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10.4,fontWeight:800,color:"#059669"}}>{paidRate}%</div>
                          </div>
                        )}
                    ...
                    <div style={{padding:"12px 16px"}}>
                      {(ph.payments||[]).length===0 ? (
                        <div style={{color:"#9CA3AF",fontSize:14.3}}>차수별 지급 정보 없음</div>
                      ) : (
                        <table style={{width:"100%",borderCollapse:"collapse"}}>
                          <thead>
                            <tr style={{background:"#F8FAFC"}}>
                              <th style={{padding:"8px 12px",textAlign:"center",fontSize:13.2,fontWeight:700,color:"#6B7280",borderBottom:"1px solid #E5E7EB"}}>차수</th>
                              <th style={{padding:"8px 12px",textAlign:"left",fontSize:13.2,fontWeight:700,color:"#6B7280",borderBottom:"1px solid #E5E7EB"}}>지급 조건</th>
                          ...
                          <tbody>
                            {(ph.payments||[]).map((p,pj)=>(
                              <tr key={pj} style={{background:pj%2===0?"#fff":"#F9FAFB"}}>
                                <td style={{padding:"8px 12px",textAlign:"center",fontSize:14.3,fontWeight:700,color:"#0E9C8C"}}>
                                  <span style={{background:"#E3F6F3",padding:"2px 8px",borderRadius:6}}>{p.round}차</span>
                                </td>
                                <td style={{padding:"8px 12px",fontSize:14.3,color:"#374151"}}>{p.condition||"-"}</td>
                                <td style={{padding:"8px 12px",textAlign:"center",fontSize:14.3,color:p.date?"#059669":"#9CA3AF",fontWeight:p.date?600:400}}>
                                  {p.date||"미정"}
                                </td>
                                <td style={{padding:"8px 12px",textAlign:"right",fontSize:14.8,fontWeight:700,color:p.amount>0?"#0B6E63":"#9CA3AF"}}>
                            ...
                            <tr style={{background:"#E3F6F3",borderTop:"2px solid #0E9C8C"}}>
                              <td style={{padding:"9px 12px",textAlign:"center",fontSize:14.3,fontWeight:800,color:"#312E81"}} colSpan={3}>합계</td>
                              <td style={{padding:"9px 12px",textAlign:"right",fontSize:15.4,fontWeight:900,color:"#0B6E63"}}>
                                {ph.paidSum>0?ph.paidSum.toLocaleString():paidAmt>0?paidAmt.toLocaleString():"-"}
                              </td>
                              <td style={{padding:"9px 12px",textAlign:"right",fontSize:14.3,fontWeight:700,color:"#0B6E63"}}>
                      ...
                      {ph.note&&<div style={{marginTop:8,fontSize:13.2,color:"#9CA3AF"}}>비고: {ph.note}</div>}
                    </div>
                  </details>
                )
              })}
          ...
          <SCard title="📝 수동 지급 등록" note="직접 입력한 지급 내역">
            ...
            {myPayments.length===0
              ?<div style={{padding:"20px",textAlign:"center",color:"#9CA3AF",fontSize:14.3}}>수동 등록된 지급 기록이 없습니다.</div>
              :<table style={{width:"100%",borderCollapse:"collapse"}}>
                <thead><tr>
                ...
                <tbody>{myPayments.map((p,i)=>(
                  <tr key={p.id} style={{background:i%2===0?"#fff":"#FAFAFA"}}>
                    ...
                    <td style={S.td("center")}>{canWrite&&<button onClick={()=>setVendorPayments(prev=>(prev||[]).filter(x=>x.id!==p.id))} style={{background:"none",border:"none",cursor:"pointer",color:"#EF4444",fontSize:15.4}}>✕</button>}</td>
                  </tr>
                ))}</tbody>
              </table>
            }
...
function DocUploadForm({onAdd,cats}) {
  ...
  const S2={inp:{padding:"8px 11px",border:"1.5px solid #E5E7EB",borderRadius:9,fontSize:14.8,fontFamily:"inherit",outline:"none",boxSizing:"border-box",width:"100%"}}
  return(
    <div style={{background:"#EEF3FF",borderRadius:12,padding:"14px 16px",marginBottom:14,border:"1px solid #3B72F633"}}>
      <div style={{fontSize:14.8,fontWeight:700,color:"#3B72F6",marginBottom:10}}>📎 문서 추가</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:10}}>
        <div><label style={{fontSize:13.2,fontWeight:700,color:"#6B7280",display:"block",marginBottom:4}}>문서명 *</label><input value={title} onChange={e=>setTitle(e.target.value)} placeholder="예: 사업자등록증 2026" style={S2.inp}/></div>
        <div><label style={{fontSize:13.2,fontWeight:700,color:"#6B7280",display:"block",marginBottom:4}}>분류</label><select value={cat} onChange={e=>setCat(e.target.value)} style={S2.inp}>{cats.map(c=><option key={c}>{c}</option>)}</select></div>
        <div><label style={{fontSize:13.2,fontWeight:700,color:"#6B7280",display:"block",marginBottom:4}}>날짜</label><input type="date" value={date} onChange={e=>setDate(e.target.value)} style={S2.inp}/></div>
        <div style={{gridColumn:"span 2"}}><label style={{fontSize:13.2,fontWeight:700,color:"#6B7280",display:"block",marginBottom:4}}>메모</label><input value={memo} onChange={e=>setMemo(e.target.value)} placeholder="추가 메모" style={S2.inp}/></div>
        <div><label style={{fontSize:13.2,fontWeight:700,color:"#6B7280",display:"block",marginBottom:4}}>파일 첨부</label>
          <div onClick={()=>fileRef.current?.click()} style={{border:"1.5px dashed #3B72F6",borderRadius:9,padding:"8px 12px",cursor:"pointer",textAlign:"center",background:"#fff",fontSize:14.3}}>
      ...
      <button onClick={add} style={{padding:"9px 20px",background:"#3B72F6",color:"#fff",border:"none",borderRadius:10,fontSize:14.8,fontWeight:700,cursor:"pointer"}}>+ 문서 추가</button>
    </div>
  )
}
    ...
    <div>
      <SCard title="📊 수주형태별 외주비(평당단가) 비교" note="동일 분야의 협력업체 계약 평당단가를 민간수의/제안공모/경쟁설계 등 수주형태별로 비교합니다.">
        <div style={{marginBottom:12}}>
          ...
          {cat&&<span style={{fontSize:13.2,color:C.gray,marginLeft:10}}>면적기준: {basis==="1식"?"1식 (평당단가 비교 불가)":basis}</span>}
        </div>

        {byType.length===0
          ? <div style={{padding:"12px 14px",borderRadius:10,background:C.grayL,color:C.gray,fontSize:14.3}}>해당 분야는 평당단가 비교가 가능한 데이터가 없습니다. (면적기준이 "1식"인 분야는 비교 대상에서 제외됩니다.)</div>
          : <>
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={byType.map(r=>({name:r.type,평균평당단가:Math.round(r.avg)}))} barCategoryGap="30%" margin={{top:24,right:10,left:0,bottom:0}}>
                ...
                <XAxis dataKey="name" tick={{fontSize:13.2,fontWeight:600}}/>
                <YAxis tick={{fontSize:11}} tickFormatter={v=>(v/10000).toFixed(0)+"만"}/>
                <Tooltip formatter={v=>v.toLocaleString()+"원/평"}/>
                <Bar dataKey="평균평당단가" fill={C.navyM} radius={[5,5,0,0]} barSize={64}>
                  <LabelList dataKey="평균평당단가" position="top" formatter={v=>(v/10000).toFixed(0)+"만"} style={{fontSize:13.2,fontWeight:700,fill:C.navyM}}/>
                </Bar>
              </BarChart>
            </ResponsiveContainer>
...
function VendorDraft({projects,setProjects,directory,canWrite,setTab,setSelProjId,setSelVerIdx}) {
  ...
  return (
    <SCard title="📝 외주비 실행초안 생성" note="대상 프로젝트와 동일한 수주형태(민간수의/제안공모/경쟁설계 등)의 다른 프로젝트 사례에서 평당단가 중간값을 산출해 제안 계약금액을 만듭니다.">
      ...
      {!proj ? <div style={{color:C.gray,fontSize:14.3}}>프로젝트를 선택하세요.</div> : basis==="1식"
        ? <div style={{padding:"12px 14px",borderRadius:10,background:C.grayL,color:C.gray,fontSize:14.3}}>"{cat}" 분야는 면적기준이 1식이라 평당단가 기반 초안을 생성할 수 없습니다.</div>
        : comparable.length===0
        ? <div style={{padding:"12px 14px",borderRadius:10,background:C.grayL,color:C.gray,fontSize:14.3}}>"{proj.bidType||"기타"}" 수주형태의 "{cat}" 분야 비교 사례가 없습니다. (다른 수주형태 데이터만 존재)</div>
        : <>
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:14}}>
            <div style={S.card({marginBottom:0,padding:"12px 14px"})}>
              <div style={{fontSize:12,color:C.gray,marginBottom:5}}>비교 사례 수 ({proj.bidType||"기타"})</div>
              <div style={{fontSize:22,fontWeight:800}}>{comparable.length}건</div>
            </div>
            <div style={S.card({marginBottom:0,padding:"12px 14px"})}>
              <div style={{fontSize:12,color:C.gray,marginBottom:5}}>평당단가 중간값</div>
              <div style={{fontSize:22,fontWeight:800,color:C.navyM}}>{Math.round(med).toLocaleString()}원</div>
            </div>
            <div style={S.card({marginBottom:0,padding:"12px 14px"})}>
              <div style={{fontSize:12,color:C.gray,marginBottom:5}}>대상 프로젝트 면적</div>
              <div style={{fontSize:22,fontWeight:800}}>{py.toLocaleString(undefined,{maximumFractionDigits:0})}평</div>
              <div style={{fontSize:11,color:C.gray}}>{basis} 기준</div>
            </div>
            <div style={S.card({marginBottom:0,padding:"12px 14px",background:C.navyL})}>
              <div style={{fontSize:12,color:C.navyM,marginBottom:5}}>제안 계약금액</div>
              <div style={{fontSize:22,fontWeight:800,color:C.navyM}}>{suggested!=null?fW(suggested):"-"}</div>
            </div>
          </div>
          ...
          {canWrite
            ...
            : <div style={{fontSize:13.2,color:C.gray}}>초안 생성은 입력 권한이 있는 계정에서 가능합니다.</div>}
          <div style={{fontSize:12,color:C.gray,marginTop:8}}>초안 생성 시 프로젝트 탭의 새 버전으로 추가되며, 업체명은 "(업체 선택 필요)"로 표시되어 직접 지정해야 합니다.</div>
        </>}
    </SCard>
  )
}
...
- 비교 분석 시 장단점, 특이사항도 포함하세요`
  ...
  const renderMD = (text) => {
    ...
    return lines.map((line,i)=>{
      if(line.startsWith("## ")) return <div key={i} style={{fontSize:17.6,fontWeight:800,color:C.navyM,margin:"10px 0 4px"}}>{line.slice(3)}</div>
      if(line.startsWith("# "))  return <div key={i} style={{fontSize:19.8,fontWeight:900,color:"#111827",margin:"12px 0 4px"}}>{line.slice(2)}</div>
      ...
      if(line.startsWith("|")&&line.includes("|")) {
        ...
        return <div key={i} style={{display:"flex",gap:0,borderBottom:"1px solid #E5E7EB"}}>
          {cells.map((c,ci)=><div key={ci} style={{flex:1,padding:"5px 8px",fontSize:14.3,borderRight:"1px solid #E5E7EB",fontWeight:cells[0].includes("---")?"normal":ci===0?700:400}}>{c.trim()==="-".repeat(c.trim().length)?null:c.trim()}</div>)}
        </div>
      }
  ...
  return (
    <SCard title="🤖 AI 통합 분석" note="프로젝트·협력업체·외주비 데이터를 자연어로 질의하여 분석합니다">
      ...
      <div style={{marginBottom:14}}>
        <div style={{fontSize:14.3,fontWeight:700,color:C.gray,marginBottom:7}}>💡 예시 질문 (클릭하면 바로 질의)</div>
          ...
          {examples.map((ex,i)=>(
            <button key={i} onClick={()=>sendQuery(ex)}
              style={{padding:"6px 12px",background:C.navyL,color:C.navyM,border:`1px solid ${C.navyM}40`,
                borderRadius:8,fontSize:13.8,cursor:"pointer",fontWeight:500,textAlign:"left",lineHeight:1.4}}>
              {ex}
            </button>
          ))}
        </div>
      ...
      <div style={{background:"#F8FAFC",borderRadius:12,border:"1px solid #E5E7EB",minHeight:300,maxHeight:500,overflowY:"auto",padding:"14px 16px",marginBottom:12}}>
        {history.length===0&&(
          <div style={{textAlign:"center",padding:"60px 20px",color:C.gray}}>
            ...
            <div style={{fontSize:15.4,fontWeight:600,marginBottom:4}}>AI 분석 어시스턴트</div>
            <div style={{fontSize:13.8}}>위 예시를 클릭하거나 아래 입력창에 질문을 입력하세요</div>
          </div>
        )}
        {history.map((msg,i)=>(
          <div key={i} style={{marginBottom:14,display:"flex",flexDirection:"column",
            ...
            <div style={{
              ...
              fontSize:14.8,lineHeight:1.65
            }}>
              {msg.role==="assistant" ? renderMD(msg.content) : msg.content}
            </div>
            {msg.time&&<div style={{fontSize:12,color:C.gray,marginTop:3}}>{msg.time}</div>}
          </div>
        ))}
        {loading&&(
          <div style={{display:"flex",alignItems:"center",gap:8,padding:"10px 14px",background:"#fff",borderRadius:12,border:"1px solid #E5E7EB",width:"fit-content"}}>
            <div style={{fontSize:19.8}}>🤖</div>
            <div style={{fontSize:14.3,color:C.gray}}>분석 중...</div>
      ...
      <div style={{display:"flex",gap:8}}>
        <input value={query} onChange={e=>setQuery(e.target.value)}
          ...
          style={{flex:1,padding:"11px 16px",border:`1.5px solid ${C.navyM}`,borderRadius:10,fontSize:15.4,fontFamily:"inherit",outline:"none"}}
          disabled={loading}/>
        <button onClick={()=>sendQuery(query)} disabled={loading||!query.trim()}
          style={{...S.btn(loading?C.gray:C.navyM),padding:"11px 22px",fontSize:15.4,opacity:loading||!query.trim()?0.6:1}}>
          전송
        </button>
        {history.length>0&&<button onClick={()=>setHistory([])}
          style={{...S.btn(C.grayL,C.gray),padding:"11px 14px",fontSize:14.3}}>
          초기화
        </button>}
      </div>
...
export function VendorPaymentPlanEditor({vendorsDB, setVendorsDB, entry, canWrite}) {
  ...
  const INP = {padding:"7px 10px",border:"1.5px solid #E5E7EB",borderRadius:7,fontSize:14.3,
          ...
          {history.map((h,i)=>(
            <button key={i} onClick={()=>setSelProjIdx(i)}
              style={{padding:"6px 12px",border:`1.5px solid ${selProjIdx===i?"#0E9C8C":"#E5E7EB"}`,
                borderRadius:8,fontSize:13.8,cursor:"pointer",fontWeight:selProjIdx===i?700:400,
      ...
      {ph && (
        <div>
          ...
          <div style={{background:"linear-gradient(135deg,#0C447C,#0B6E63)",borderRadius:12,padding:"14px 18px",marginBottom:14,color:"#fff",display:"flex",gap:16,flexWrap:"wrap",alignItems:"center"}}>
            <div style={{flex:1}}>
              <div style={{fontSize:12.6,opacity:.8,marginBottom:3}}>[{ph.type}] {ph.project?.slice(0,40)}</div>
              <div style={{display:"flex",gap:16}}>
                <div><div style={{fontSize:11,opacity:.7}}>총 용역금액</div><div style={{fontSize:16.5,fontWeight:800}}>{(ph.totalAmt/1e8).toFixed(2)}억</div></div>
                <div><div style={{fontSize:11,opacity:.7}}>지급 합계</div><div style={{fontSize:16.5,fontWeight:800,color:"#34D399"}}>{ph.paidSum>0?(ph.paidSum/1e8).toFixed(2)+"억":"-"}</div></div>
                <div><div style={{fontSize:11,opacity:.7}}>잔액</div><div style={{fontSize:16.5,fontWeight:800,color:ph.remain>0?"#FDE68A":"#9CA3AF"}}>{ph.remain>0?(ph.remain/1e8).toFixed(2)+"억":"-"}</div></div>
              </div>
            </div>
          </div>
            ...
            <thead>
              <tr style={{background:"#F8FAFC"}}>
                {["차수","지급 조건/단계","지급 예정일","금액(원)","지급 여부","비고",""].map((h,i)=>(
                  <th key={h+i} style={{padding:"9px 12px",textAlign:i>=2?"center":"left",fontSize:13.8,fontWeight:700,color:"#6B7280",borderBottom:"2px solid #E5E7EB"}}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {(ph.payments||[]).map((p,pi)=>{
                ...
                return (
                  <tr key={pi} style={{background:isPaid?"#F0FDF4":pi%2===0?"#fff":"#F9FAFB"}}>
                    <td style={{padding:"9px 12px",textAlign:"center",fontWeight:700,color:"#0E9C8C",fontSize:14.3}}>
                      <span style={{background:"#E3F6F3",padding:"2px 8px",borderRadius:6}}>{p.round}차</span>
                    </td>
                    <td style={{padding:"9px 12px",fontSize:14.3,color:"#374151",minWidth:120}}>
                      {isEdit
                    ...
                    <td style={{padding:"9px 12px",textAlign:"center",minWidth:130}}>
                      {isEdit
                        ...
                        : <span style={{fontSize:14.3,fontWeight:p.date?700:400,color:p.date?"#059669":"#9CA3AF"}}>{p.date||"미정"}</span>}
                    </td>
                    <td style={{padding:"9px 12px",textAlign:"right",minWidth:120}}>
                      {isEdit
                        ...
                        : <span style={{fontSize:14.8,fontWeight:700,color:p.amount>0?"#0B6E63":"#9CA3AF"}}>{p.amount>0?p.amount.toLocaleString():"-"}</span>}
                    </td>
                    ...
                    <td style={{padding:"9px 12px",fontSize:13.2,color:"#9CA3AF",minWidth:100}}>
                      {isEdit
                    ...
                    <td style={{padding:"6px 8px",textAlign:"center",whiteSpace:"nowrap"}}>
                      {canWrite && !isEdit && (
                        <button onClick={()=>{setEditingIdx(pi);setDraft({...p})}}
                          style={{padding:"4px 10px",background:"#E3F6F3",color:"#0E9C8C",border:"none",borderRadius:6,fontSize:12.6,cursor:"pointer",fontWeight:700}}>✏</button>
                      )}
                      {canWrite && isEdit && (
                        <div style={{display:"flex",gap:4}}>
                          ...
                          }} style={{padding:"4px 10px",background:"#D1FAE5",color:"#059669",border:"none",borderRadius:6,fontSize:12.6,cursor:"pointer",fontWeight:700}}>✓</button>
                          <button onClick={()=>{setEditingIdx(null);setDraft(null)}}
                            style={{padding:"4px 8px",background:"#F3F4F6",color:"#6B7280",border:"none",borderRadius:6,fontSize:12.6,cursor:"pointer"}}>✕</button>
                          <button onClick={()=>{
                            if(!window.confirm("이 차수를 삭제하시겠습니까?")) return
                            const updated=(ph.payments||[]).filter((_,i)=>i!==pi)
                            savePaymentPlan(selProjIdx,updated)
                            setEditingIdx(null); setDraft(null)
                          }} style={{padding:"4px 8px",background:"#FEE2E2",color:"#DC2626",border:"none",borderRadius:6,fontSize:12.6,cursor:"pointer"}}>🗑</button>
                        </div>
                      )}
                    </td>
                  </tr>
              ...
              {/* 합계 행 */}
              <tr style={{background:"#E3F6F3",borderTop:"2px solid #0E9C8C"}}>
                <td style={{padding:"9px 12px",fontWeight:800,color:"#312E81",textAlign:"center",fontSize:14.3}} colSpan={3}>합계</td>
                <td style={{padding:"9px 12px",textAlign:"right",fontWeight:900,color:"#0B6E63",fontSize:15.4}}>
            ...
            <div>
              {!showAdd
                ? <button onClick={()=>setShowAdd(true)}
                    style={{padding:"8px 18px",background:"#0E9C8C",color:"#fff",border:"none",borderRadius:9,fontSize:14.3,fontWeight:700,cursor:"pointer"}}>
                    + 새 차수 추가
                  </button>
                : <div style={{background:"#E3F6F3",borderRadius:12,border:"2px solid #0E9C8C",padding:"14px 16px"}}>
                    <div style={{fontSize:14.8,fontWeight:700,color:"#312E81",marginBottom:10}}>+ 지급 차수 추가</div>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 2fr 1fr 1fr 1fr",gap:10,marginBottom:10}}>
                      <div>
                        <label style={{fontSize:12,fontWeight:700,color:"#0E9C8C",display:"block",marginBottom:3}}>차수</label>
                        <input type="number" value={newPay.round} onChange={e=>setNewPay(p=>({...p,round:e.target.value}))}
                          placeholder={((ph.payments||[]).length+1)+"차"}
                          style={INP}/>
                      </div>
                      <div>
                        <label style={{fontSize:12,fontWeight:700,color:"#0E9C8C",display:"block",marginBottom:3}}>지급 조건/단계</label>
                        <input value={newPay.condition} onChange={e=>setNewPay(p=>({...p,condition:e.target.value}))}
                          placeholder="예: 실시설계 완료 후" style={INP}/>
                      </div>
                      <div>
                        <label style={{fontSize:12,fontWeight:700,color:"#0E9C8C",display:"block",marginBottom:3}}>지급 예정일</label>
                        <input type="date" value={newPay.date} onChange={e=>setNewPay(p=>({...p,date:e.target.value}))} style={INP}/>
                      </div>
                      <div>
                        <label style={{fontSize:12,fontWeight:700,color:"#0E9C8C",display:"block",marginBottom:3}}>금액(원)</label>
                        <input type="number" value={newPay.amount} onChange={e=>setNewPay(p=>({...p,amount:e.target.value}))}
                          placeholder="10000000" style={INP}/>
                      </div>
                      <div>
                        <label style={{fontSize:12,fontWeight:700,color:"#0E9C8C",display:"block",marginBottom:3}}>비고</label>
                    ...
                    <div style={{display:"flex",gap:8}}>
                      <button onClick={()=>{
                        ...
                        setShowAdd(false)
                      }} style={{padding:"7px 18px",background:"#059669",color:"#fff",border:"none",borderRadius:9,fontSize:14.3,fontWeight:700,cursor:"pointer"}}>💾 추가</button>
                      <button onClick={()=>setShowAdd(false)}
                        style={{padding:"7px 14px",background:"#F3F4F6",color:"#6B7280",border:"none",borderRadius:9,fontSize:14.3,cursor:"pointer"}}>취소</button>
                    </div>
                  </div>
              }
            </div>
