    ...
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:0}}>
      <div style={{position:"relative",display:"flex",flexDirection:"column",alignItems:"center"}}>
        ...
        <div style={{
          ...
          <div style={{fontSize:15.4,fontWeight:800}}>{node.title}</div>
          <div style={{fontSize:12,opacity:.85,marginTop:2}}>{node.role}</div>
          ...
          {hasDeptMembers&&(
            <div onClick={e=>{e.stopPropagation();setShowMembers(v=>!v)}}
              style={{position:"absolute",top:-8,left:-8,minWidth:20,height:20,borderRadius:10,
                ...
                fontSize:11.6,fontWeight:800,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",
          ...
          <div style={{position:"absolute",top:-8,right:-8,display:"flex",gap:2}}>
            <button onClick={e=>{e.stopPropagation();onEdit(node)}}
              style={{width:18,height:18,border:"none",borderRadius:"50%",background:"#FEF3C7",color:"#D97706",fontSize:10,cursor:"pointer",fontWeight:900,display:"flex",alignItems:"center",justifyContent:"center"}}>✏</button>
            <button onClick={e=>{e.stopPropagation();onAdd(node)}}
              style={{width:18,height:18,border:"none",borderRadius:"50%",background:"#D1FAE5",color:"#059669",fontSize:13.2,cursor:"pointer",fontWeight:900,display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
            {depth>0&&<button onClick={e=>{e.stopPropagation();onDelete(node.id)}}
              style={{width:18,height:18,border:"none",borderRadius:"50%",background:"#FEE2E2",color:"#DC2626",fontSize:10,cursor:"pointer",fontWeight:900,display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>}
          </div>
          ...
          {depth>0&&(
            <div style={{position:"absolute",bottom:-8,left:"50%",transform:"translateX(-50%)",display:"flex",gap:2,zIndex:2}}>
              {!isFirst&&<button onClick={e=>{e.stopPropagation();onMove(node.id,"left")}}
                style={{width:16,height:16,border:"none",borderRadius:"50%",background:"#E3F6F3",color:"#0E9C8C",fontSize:8.8,cursor:"pointer",fontWeight:900,display:"flex",alignItems:"center",justifyContent:"center"}}>←</button>}
        ...
        {showMembers&&hasDeptMembers&&(
          <div style={{position:"absolute",top:"100%",left:"50%",transform:"translateX(-50%)",
            ...
            <div style={{padding:"6px 14px 8px",borderBottom:"1px solid #F3F4F6",
              fontSize:14.3,fontWeight:800,color:node.color,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <span>👥 {node.title} ({members.length}명)</span>
              <button onClick={e=>{e.stopPropagation();setShowMembers(false)}}
                style={{border:"none",background:"none",cursor:"pointer",fontSize:15.4,color:"#9CA3AF"}}>✕</button>
            </div>
                ...
                <div key={m.id||i} style={{padding:"8px 14px",borderBottom:"1px solid #F9FAFB",
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{display:"flex",gap:5,alignItems:"center",marginBottom:2}}>
                      <span style={{fontSize:15.4,fontWeight:800,color:"#111827"}}>{m.name}</span>
                      <span style={{fontSize:11,padding:"1px 5px",borderRadius:6,
                        background:(STATUS_BG[m.status]||"#F3F4F6"),
                        color:(STATUS_COLOR[m.status]||"#6B7280"),fontWeight:700}}>
                        {m.status}
                      </span>
                    </div>
                    <div style={{fontSize:13.2,color:"#6B7280",fontWeight:500}}>{m.rank}</div>
                    {m.mobile&&<div style={{fontSize:13.2,color:"#0E9C8C",fontWeight:600}}>{m.mobile}</div>}
                  </div>
                </div>
              ))}
            </div>
...
function OrgChart({org, setOrg, staffList=[]}) {
  ...
  const INP={padding:"7px 10px",border:"1.5px solid #E5E7EB",borderRadius:7,fontSize:14.3,width:"100%",boxSizing:"border-box",fontFamily:"inherit",outline:"none"}

  return (
    <div>
      <div style={{display:"flex",gap:8,marginBottom:12,alignItems:"center",flexWrap:"wrap"}}>
        <div style={{fontSize:17.6,fontWeight:800,color:"#312E81"}}>🏢 조직도</div>
        <div style={{fontSize:13.2,color:"#9CA3AF",marginLeft:4}}>
          ← → 버튼으로 노드 순서 이동 · 숫자 뱃지 클릭으로 소속 직원 확인
        </div>
        <button onClick={()=>{if(window.confirm("조직도를 초기화하시겠습니까?")) save(DEFAULT_ORG)}}
          style={{marginLeft:"auto",padding:"5px 12px",background:"#F3F4F6",color:"#6B7280",border:"none",borderRadius:7,fontSize:13.2,cursor:"pointer"}}>
          🔄 초기화
        </button>
      </div>
      ...
      {(editNode||addParentId)&&draftNode&&(
        <div style={{background:"#E3F6F3",borderRadius:12,border:"2px solid #0E9C8C",padding:"14px 16px",marginBottom:14}}>
          <div style={{fontSize:14.8,fontWeight:700,color:"#312E81",marginBottom:10}}>
            {editNode?"✏ 노드 수정":"+ 하위 노드 추가"}
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
            <div>
              <label style={{fontSize:12,fontWeight:700,color:"#0E9C8C",display:"block",marginBottom:3}}>이름/부서명</label>
              <input value={draftNode.title} onChange={e=>setDraftNode(p=>({...p,title:e.target.value}))} style={INP}/>
            </div>
            <div>
              <label style={{fontSize:12,fontWeight:700,color:"#0E9C8C",display:"block",marginBottom:3}}>직책/역할</label>
              <input value={draftNode.role||""} onChange={e=>setDraftNode(p=>({...p,role:e.target.value}))} style={INP}/>
            </div>
          </div>
          <div style={{marginBottom:10}}>
            <label style={{fontSize:12,fontWeight:700,color:"#0E9C8C",display:"block",marginBottom:4}}>색상</label>
          ...
          <div style={{display:"flex",gap:8}}>
            <button onClick={()=>{setEditNode(null);setAddParentId(null);setDraftNode(null)}}
              style={{padding:"6px 14px",background:"#F3F4F6",color:"#6B7280",border:"none",borderRadius:7,fontSize:13.8,cursor:"pointer"}}>취소</button>
            <button onClick={saveNode}
              style={{padding:"6px 16px",background:"#0E9C8C",color:"#fff",border:"none",borderRadius:7,fontSize:13.8,fontWeight:700,cursor:"pointer"}}>💾 저장</button>
          </div>
        </div>
      )}
...
export function StaffMgmtPage({currentUser,deptStaff,setDeptStaff,DEPTS=[],DEPT_COLORS={},setTab}){
  ...
  const INP={padding:"8px 12px",border:"1.5px solid #E5E7EB",borderRadius:8,fontSize:15.4,fontFamily:"inherit",outline:"none",width:"100%",boxSizing:"border-box"}
  const LBL={fontSize:13.2,fontWeight:700,color:"#0E9C8C",display:"block",marginBottom:4}
  ...
  return (
    <div style={{fontFamily:"'Noto Sans KR',sans-serif"}}>
      ...
      <div style={{background:"linear-gradient(135deg,#312E81,#0E9C8C)",borderRadius:16,padding:"18px 22px",marginBottom:14,color:"#fff",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10}}>
        <div>
          <div style={{fontSize:22,fontWeight:900,marginBottom:3}}>👤 직원 관리 시스템</div>
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
          <button onClick={()=>setTab&&setTab("home")}
            style={{padding:"7px 14px",background:"rgba(255,255,255,.2)",color:"#fff",border:"2px solid rgba(255,255,255,.35)",borderRadius:9,fontSize:13.8,fontWeight:700,cursor:"pointer"}}>🏠 홈</button>
          <button onClick={downloadTemplate}
            style={{padding:"7px 14px",background:"#D1FAE5",color:"#065F46",border:"none",borderRadius:9,fontSize:13.8,fontWeight:700,cursor:"pointer"}}>⬇ 양식</button>
          <button onClick={downloadAll}
            style={{padding:"7px 14px",background:"#EDE9FE",color:"#5B21B6",border:"none",borderRadius:9,fontSize:13.8,fontWeight:700,cursor:"pointer"}}>⬇ 전체</button>
          <label style={{padding:"7px 14px",background:"#FEF3C7",color:"#92400E",border:"none",borderRadius:9,fontSize:13.8,fontWeight:700,cursor:"pointer",display:"inline-flex",alignItems:"center",gap:3}}>
            ⬆ 업로드 <input type="file" accept=".xlsx,.xls" style={{display:"none"}} onChange={uploadExcel}/>
          </label>
          <button onClick={startAdd}
            style={{padding:"7px 14px",background:"#fff",color:"#0E9C8C",border:"none",borderRadius:9,fontSize:13.8,fontWeight:800,cursor:"pointer"}}>+ 추가</button>
          <button onClick={()=>{if(window.confirm("초기화하시겠습니까?")){localStorage.removeItem("sjs_staff_db");window.location.reload()}}}
            style={{padding:"7px 12px",background:"rgba(255,255,255,.1)",color:"rgba(255,255,255,.7)",border:"1px solid rgba(255,255,255,.2)",borderRadius:9,fontSize:12,cursor:"pointer"}}>🔄</button>
        </div>
      </div>
      ...
      <div style={{display:"flex",gap:4,marginBottom:14,background:"#F3F4F6",borderRadius:10,padding:3,width:"fit-content"}}>
        {[["org","🏢 조직도"],["list","👥 직원 목록"]].map(([v,l])=>(
          <button key={v} onClick={()=>{setView(v);if(v==="org")setSelId(null)}}
            style={{padding:"8px 20px",border:"none",borderRadius:8,fontSize:14.8,fontWeight:view===v?700:400,cursor:"pointer",
        ...
        <div>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:12}}>
            <div onClick={()=>setFilter(p=>({...p,dept:"전체"}))}
              ...
              <span style={{fontSize:14.8,fontWeight:700,color:filter.dept==="전체"?"#fff":"#6B7280"}}>전체</span>
              <span style={{fontSize:19.8,fontWeight:900,color:filter.dept==="전체"?"#fff":"#111827"}}>
            ...
            {Object.entries(deptStats).sort((a,b)=>b[1]-a[1]).map(([d,c])=>(
              <div key={d} onClick={()=>setFilter(p=>({...p,dept:d}))}
                ...
                <span style={{fontSize:14.3,fontWeight:700,color:filter.dept===d?"#fff":(DEPT_COLORS[d]||"#0E9C8C")}}>{d}</span>
                <span style={{fontSize:19.8,fontWeight:900,color:filter.dept===d?"#fff":"#111827"}}>{c}</span>
              </div>
            ))}
          </div>
          ...
          <div style={{display:"flex",gap:12}}>
            ...
            <div style={{width:290,flexShrink:0}}>
              <div style={{background:"#fff",borderRadius:12,border:"1px solid #E5E7EB",padding:"10px",marginBottom:8}}>
                ...
                <div style={{fontSize:13.2,color:"#6B7280",marginTop:6,fontWeight:600}}>{filtered.length}명</div>
              </div>
                ...
                {filtered.map(s=>(
                  <div key={s.id} onClick={()=>{setSelId(s.id);setShowAdd(false)}}
                    ...
                    <div style={{width:40,height:40,borderRadius:"50%",flexShrink:0,background:s.photo?"transparent":"#E5E7EB",
                      ...
                      {s.photo?<img src={s.photo} style={{width:"100%",height:"100%",objectFit:"cover"}}/>:<span style={{fontSize:18.7,color:"#9CA3AF"}}>👤</span>}
                    </div>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:"flex",gap:4,alignItems:"center",marginBottom:1}}>
                        <span style={{fontSize:15.4,fontWeight:800,color:"#111827"}}>{s.name}</span>
                        <span style={{fontSize:10,padding:"1px 5px",borderRadius:7,background:STATUS_BG[s.status]||"#F3F4F6",color:STATUS_COLOR[s.status]||"#6B7280",fontWeight:700}}>{s.status}</span>
                        {s.excludeCount&&<span style={{fontSize:10,padding:"1px 5px",borderRadius:7,background:"#F3F4F6",color:"#9CA3AF",fontWeight:700}}>제외</span>}
                      </div>
                      <div style={{fontSize:12.6,color:"#6B7280",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.dept}·{s.rank}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div style={{flex:1}}>
              {showAdd&&draft&&(
                <div style={{background:"#fff",borderRadius:14,border:"2px solid #0E9C8C",padding:"20px",marginBottom:12}}>
                  <div style={{fontSize:17.6,fontWeight:800,color:"#312E81",marginBottom:14}}>{draft.id?"✏ 정보 수정":"+ 신규 등록"}</div>
                  ...
                  <label style={{display:"flex",gap:8,alignItems:"center",marginBottom:12,cursor:"pointer",fontSize:14.8,fontWeight:600,color:"#374151"}}>
                  ...
                  <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
                    <button onClick={()=>{setShowAdd(false);setDraft(null)}}
                      style={{padding:"8px 16px",background:"#F3F4F6",color:"#6B7280",border:"none",borderRadius:9,fontSize:14.3,fontWeight:600,cursor:"pointer"}}>취소</button>
                    <button onClick={saveDraft}
                      style={{padding:"8px 20px",background:"#0E9C8C",color:"#fff",border:"none",borderRadius:9,fontSize:14.3,fontWeight:800,cursor:"pointer"}}>💾 저장</button>
                  </div>
                </div>
              )}
              ...
              {sel&&!showAdd&&(
                <div style={{display:"flex",flexDirection:"column",gap:10}}>
                  <div style={{background:"#fff",borderRadius:14,border:"1px solid #E5E7EB",padding:"18px",display:"flex",gap:18,alignItems:"flex-start"}}>
                    <div style={{flexShrink:0,textAlign:"center"}}>
                      <div style={{width:90,height:90,borderRadius:14,background:"#E5E7EB",overflow:"hidden",display:"flex",alignItems:"center",justifyContent:"center",border:"3px solid #E5E7EB",marginBottom:6}}>
                        {sel.photo?<img src={sel.photo} style={{width:"100%",height:"100%",objectFit:"cover"}}/>:<span style={{fontSize:44}}>👤</span>}
                      </div>
                      <input type="file" ref={photoRef} accept="image/*" style={{display:"none"}} onChange={handlePhoto}/>
                      <button onClick={()=>photoRef.current?.click()}
                        style={{padding:"4px 10px",background:"#E3F6F3",color:"#0E9C8C",border:"none",borderRadius:6,fontSize:12.6,cursor:"pointer",fontWeight:700}}>📷 사진</button>
                    </div>
                    <div style={{flex:1}}>
                      <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:6,flexWrap:"wrap"}}>
                        <span style={{fontSize:22,fontWeight:900,color:"#111827"}}>{sel.name}</span>
                        {sel.nameEn&&<span style={{fontSize:14.3,color:"#9CA3AF"}}>{sel.nameEn}</span>}
                        <span style={{fontSize:12,padding:"2px 9px",borderRadius:9,background:STATUS_BG[sel.status]||"#F3F4F6",color:STATUS_COLOR[sel.status]||"#6B7280",fontWeight:800}}>{sel.status}</span>
                        {sel.excludeCount&&<span style={{fontSize:12,padding:"2px 9px",borderRadius:9,background:"#F3F4F6",color:"#9CA3AF",fontWeight:700}}>인원제외</span>}
                      </div>
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"5px 18px",fontSize:14.8,marginBottom:10}}>
                      ...
                      <div style={{display:"flex",gap:7}}>
                        <button onClick={()=>startEdit(sel)}
                          style={{padding:"6px 14px",background:"#0E9C8C",color:"#fff",border:"none",borderRadius:9,fontSize:14.3,fontWeight:700,cursor:"pointer"}}>✏ 수정</button>
                        <button onClick={()=>{if(window.confirm("삭제?")){setStaffDB(p=>{const n={...p};delete n[sel.id];return n});setSelId(null)}}}
                          style={{padding:"6px 14px",background:"#FEE2E2",color:"#DC2626",border:"none",borderRadius:9,fontSize:14.3,fontWeight:700,cursor:"pointer"}}>🗑 삭제</button>
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={{background:"#fff",borderRadius:14,border:"1px solid #E5E7EB",overflow:"hidden"}}>
                    <div style={{padding:"12px 16px",borderBottom:"1px solid #E5E7EB",fontSize:16.5,fontWeight:800,color:"#111827"}}>
                    ...
                    <div style={{padding:"10px 14px",background:"#F9FAFB",borderBottom:"1px solid #E5E7EB"}}>
                        ...
                        {MEMO_TYPES.map(t=>(
                          <button key={t} onClick={()=>setMemoType(t)}
                            style={{padding:"3px 10px",border:`2px solid ${memoType===t?(MEMO_COLOR[t]||"#0E9C8C"):"#E5E7EB"}`,
                              borderRadius:7,fontSize:13.2,cursor:"pointer",fontWeight:memoType===t?700:400,
                      ...
                      <div style={{display:"grid",gridTemplateColumns:"140px 1fr auto",gap:6,alignItems:"center"}}>
                        <input type="date" value={memoDate} onChange={e=>setMemoDate(e.target.value)}
                          style={{...INP,fontSize:13.8}}/>
                        <input value={newMemo} onChange={e=>setNewMemo(e.target.value)}
                          onKeyDown={e=>e.key==="Enter"&&addMemo()}
                          placeholder={`[${memoType}] 내용 입력`}
                          style={INP}/>
                        <button onClick={addMemo}
                          style={{padding:"8px 16px",background:"#0E9C8C",color:"#fff",border:"none",borderRadius:9,fontSize:14.3,fontWeight:700,cursor:"pointer",flexShrink:0,whiteSpace:"nowrap"}}>+ 기록</button>
                      </div>
                    </div>
                    <div style={{maxHeight:360,overflowY:"auto"}}>
                      {(sel.memo||[]).length===0&&(
                        <div style={{padding:"36px",textAlign:"center",color:"#9CA3AF",fontSize:14.3}}>아직 기록이 없습니다.</div>
                      )}
                        ...
                        <div key={m.id||i} style={{padding:"11px 14px",borderBottom:"1px solid #F3F4F6",display:"flex",gap:10,alignItems:"flex-start",background:m.auto?"#FAFAFA":"#fff"}}>
                          ...
                          <div style={{flex:1}}>
                            <div style={{display:"flex",gap:7,alignItems:"center",marginBottom:3,flexWrap:"wrap"}}>
                              <span style={{fontSize:12,padding:"1px 6px",borderRadius:7,background:(MEMO_COLOR[m.type||"일반"]||"#0E9C8C")+"18",color:MEMO_COLOR[m.type||"일반"]||"#0E9C8C",fontWeight:700}}>{m.type||"일반"}</span>
                              <span style={{fontSize:14.3,fontWeight:800,color:"#374151"}}>{m.date}</span>
                              {m.time&&<span style={{fontSize:12.6,color:"#9CA3AF"}}>{m.time}</span>}
                              {m.author&&<span style={{fontSize:12,color:"#9CA3AF"}}>by {m.author}</span>}
                              {m.auto&&<span style={{fontSize:10,background:"#E3F6F3",color:"#0E9C8C",padding:"1px 5px",borderRadius:4}}>자동</span>}
                            </div>
                            <div style={{fontSize:14.8,color:"#111827",lineHeight:1.6}}>{m.text}</div>
                          </div>
                          {!m.auto&&<button onClick={()=>{
                            const real=[...(sel.memo||[])]; real.splice(real.length-1-i,1)
                            setStaffDB(prev=>({...prev,[sel.id]:{...sel,memo:real}}))
                          }} style={{padding:"2px 7px",background:"#FEE2E2",color:"#DC2626",border:"none",borderRadius:5,fontSize:12,cursor:"pointer",flexShrink:0}}>✕</button>}
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              {!sel&&!showAdd&&(
                <div style={{padding:"60px",textAlign:"center",color:"#9CA3AF",background:"#fff",borderRadius:14,border:"1px solid #E5E7EB"}}>
                  <div style={{fontSize:46.2,marginBottom:10}}>👤</div>
                  <div style={{fontSize:16.5,fontWeight:700,color:"#374151",marginBottom:5}}>직원을 선택하면 상세정보가 표시됩니다</div>
                  <div style={{fontSize:13.8}}>왼쪽 목록에서 이름을 클릭하세요</div>
                </div>
              )}
            </div>
          </div>
